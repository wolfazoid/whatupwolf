#!/usr/bin/env bash
# Robust cron entry for the Site Health monitor.
#
...
# moment nvm upgrades node — silently, on a Wednesday no one is watching. Instead
# the crontab points at THIS script, which sources nvm to pick up whatever node is
...
cd "$(dirname "$0")/.." || exit 1
exec node engine/run-experiment.mjs site-health >> engine/experiment.log 2>&1
