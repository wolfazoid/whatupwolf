...
- [ ] Revamp the Lab page filter UX (src/components/LabFilter.tsx + its use in src/pages/lab/index.astro): the per-tag buttons have gotten out of hand as entries pile up. Replace them with a simpler selector — a dropdown/select to filter by tag (and/or type) — AND add a text search box that filters entries by title + summary (client-side, keep the existing React-island approach, fast, accessible, on-brand). Verify `npm run build`. (Touches src/components / src/pages — GATED, needs-human.)
